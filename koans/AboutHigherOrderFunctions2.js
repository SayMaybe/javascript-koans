describe("About Higher Order Functions pt 2", function () {
  ...
  it("should make the band 3", function() {
    ...
    var totalDancingSkillLeft = bandMembers.reduce(function(acc, x) {return acc + x.dancing}, 0);
  });
  expect(totalDancingSkillLeft).toEqual(42);

});
// reduce bandMembers to get the sum of all performing properties
 var totalPerformingSkillLeft = bandMembers.reduce(function(acc, x) {return acc + x.performing}, 0);
 ...
 var bestPerformAndDance = bandMembers.reduce(function (best, m) => {
   /* FILL ME IN */
 },{maxPerform: 0, maxDance: 0});//parameters
